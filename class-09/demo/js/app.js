...
const kittenFormElem = document.getElementById('addKittenForm');
...
function handleSubmit(event) {
  // forms do this weird thing where they delete all your data????
  ...
  console.log(name, interests, isGoodWithDogs, isGoodWithCats, isGoodWithKids, photo);
  const newKitten = new Kitten(name, interests, isGoodWithDogs, isGoodWithCats, isGoodWithKids, photo);
  console.log(newKitten);
  newKitten.getAge();
  newKitten.renderKitten();
  // makes form go back to an empty state
  event.target.reset();

  // for today you wont be able to just append a new store row to your table
  // OUR FOOTER!
  // how to add the new store to the body and then make a new footer
  //might need to erase some stuff
  // store
  // store
  // store
  // footer
...
// what type of event do we want to listen to? - string
// the name of a function that tells us what to do when we hear it
// by default the addEventListener method returns an event
kittenFormElem.addEventListener('submit', handleSubmit);
